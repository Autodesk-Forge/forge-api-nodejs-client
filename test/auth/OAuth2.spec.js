...
module.exports = (function () {
  ...
  before(function () {
    var FORGE_CLIENT_ID =
      process.env.FORGE_CLIENT_ID || "<your forge client ID>";
    var FORGE_CLIENT_SECRET =
      process.env.FORGE_CLIENT_SECRET || "<your forge client secret>";
    var FORGE_CALLBACK =
      process.env.FORGE_CALLBACK || "<your forge callback url>";

    oauth2client2legged = new OAuth2TwoLegged(
      FORGE_CLIENT_ID,
      FORGE_CLIENT_SECRET,
      ["data:read", "data:write"]
    );
    oauth2client3legged = new OAuth2ThreeLegged(
      FORGE_CLIENT_ID,
      FORGE_CLIENT_SECRET,
      FORGE_CALLBACK,
      ["data:read", "data:write"]
  ...
  describe("OAuth2", function () {
    describe("OAuth2 interface", function () {
      it("should be an interface only (can't create instance)", function () {
        try {
          oauth2 = new OAuth2(FORGE_CLIENT_ID, FORGE_CLIENT_SECRET, [
            "data:read",
            "data:write",
          ]);
        } catch (e) {
          expect(e).to.eql(
            new Error(
              'Your OAuth2 object is missing the "authentication" property'
            )
          );
        }
      });
    ...
    describe("OAuth2 clients common", function () {
      ...
      it("should be able to call doPostRequestWithHeaders", function (done) {
        expect(oauth2client2legged.doPostRequestWithHeaders).to.be.a(Function);
        expect(oauth2client3legged.doPostRequestWithHeaders).to.be.a(Function);
        ...
        oauth2client2legged.doPostRequestWithHeaders(
          urlBasePath + "/foo",
          {},
          function (response) {
    ...
    describe("OAuth2 two-legged v2 client", function () {
      it("setCredentials and getCredentials should work as expected", function () {
        ...
        expect(oauth2client2legged.getCredentials()).to.eql({
          access_token: "abcd",
        });
      });
      ...
      it("isAuthorized should work as expected", function () {
        ...
        oauth2client2legged.setCredentials({
          access_token: "abcd",
      ...
      it("authenticate should work", function (done) {
        mockedPostRequest = sinon.stub(
          oauth2client2legged,
          "doPostRequestWithHeaders"
        );
        var credentials = {
          access_token: "abcdef",
          expires_in: 1800,
        ...
        oauth2client2legged.authenticate().then(
          function (response) {
    ...
    describe("OAuth2 three-legged v2 client", function () {
      ...
      it("generateAuthUrl should work as expected", function () {
        ...
        expect(oauth2client3legged.generateAuthUrl()).to.contain(
          "authentication/v2/authorize"
        );
        expect(oauth2client3legged.generateAuthUrl()).to.contain(
          "response_type=code"
        );
        expect(oauth2client3legged.generateAuthUrl("state-test")).to.contain(
          "state=state-test"
        );
      });
      ...
      it("getToken should work", function (done) {
        ...
        oauth2client3legged.getToken("some_code").then(
          function (response) {
            expect(response).to.be.ok();
            expect(response).to.have.property("expires_at");
            mockedPostRequest.restore();
      ...
      it("refreshToken should work", function (done) {
        ...
        oauth2client3legged.refreshToken(credentials).then(
          function (response) {
            expect(response).to.be.ok();
            expect(response).to.have.property("expires_at");
            mockedPostRequest.restore();
